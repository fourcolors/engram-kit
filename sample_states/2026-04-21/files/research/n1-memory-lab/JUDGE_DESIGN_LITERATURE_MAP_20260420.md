...
> **RETIRED 2026-04-21.** The 9-primitive set (4 universal + 5 conditional) originally listed here
> was never deployed. On review the author judged several of these as speculative names rather than
> primitives emerging from the data. Do not resurrect from this section. Future rubric design will
> start from scratch — from primitives that emerge from actual answer failures observed in runs
> plus explicit rubric-validity tests (schematic adherence, factor collapse, per-criterion κ).
>
> The claim-typing idea (`verified | inferred | speculative`) is also retired as a global contract;
> Autorubric's `CANNOT_ASSESS` + negative-weight anti-pattern pattern (Eq 1) is the better
> operational move when we get to rubric-v2.
>
> History preserved via git; do not re-cite this section.
